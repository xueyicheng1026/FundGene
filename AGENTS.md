...
- `services/evals` now contains the first local agent regression harness with 20 JSON cases covering safety refusal, tool selection, citation presence, beginner clarity, portfolio, behavior, simulation, and news/policy intents.
...
- Domain workers such as learning, portfolio, behavior, and news should receive structured inputs and return structured outputs; they must not freely chat with each other or directly mutate canonical user state.

...
- The 2026-05-21 Codex-inspired runtime continuation is moving module by module rather than cloning Codex. The current slice adds a backend-owned agent run status contract (`GET /api/assistant/runs/{run_id}/status`) that merges the in-process active registry with persisted `agent_runs`, adds `GET /api/assistant/runs/active` for current-user active-run discovery after refresh, removes the stale event-sink reference from the active registry, lets `/agent` show user-facing running/cancelling/completed/cancelled/failed/interrupted status text alongside persisted event replay, normalizes unexpected runtime failures into durable `failed` runs with `turn_error` plus `turn_complete(failed)` events, lets the web client recover when SSE includes run events but misses the final conversation by reading run status/session or replay event session metadata, treats an actively running status as still in progress instead of pulling a stale session conversation as if the run completed, lets recoverable SSE `error` payloads carry the latest `run_id` so the frontend can still recover from status/session before showing an error, and closes a persisted `running` run that is no longer active in the current process as `interrupted` from the status, event replay, and trace read surfaces with a user-visible marker, `turn_interrupted`, and `turn_closed(interrupted)` rather than leaving it forever running or pretending it completed. Event replay now also supports explicit cursor reads through `GET /api/assistant/runs/{run_id}/events?after_sequence=N`, while the default no-cursor call remains full-history for recovery paths that still need session metadata, streamed `agent_event` SSE blocks now include the stable event `id:` line, and the web SSE parser accepts both LF and CRLF block framing while using the SSE `id:` line as the event-id fallback when `data` omits it. The slice also adds a durable `assistant_queued_followups` path so a user can queue the next sentence while a run is active and the frontend can send it only after the previous run completes successfully. Queued input transitions now append replayable `input_queued`, `input_submitted`, and `input_discarded` events to the original run, with sequence numbers continuing after earlier run events, and the backend refuses to mark queued input as submitted unless the source run is no longer active and persisted as `completed`; submitted confirmation can also carry the exact `queued_follow_up_id`, so a stale auto-submit cannot mark a newer replacement item as sent. Cancel requests now immediately append replayable `turn_cancel_requested`, clear any queued follow-up for that run with `input_discarded`, and mark the cancellation token so the orchestrator does not duplicate the request event before it later emits `turn_aborted`; when a run actually resolves as cancelled, replay now closes with `turn_closed(cancelled)` instead of pretending the cancelled turn was a normal `turn_complete`, and cancelled/interrupted terminal payloads carry a reason plus resume guidance for later recovery. Active-run cleanup now follows the Codex terminal-event ordering more closely: the process-local registry entry remains visible through terminal DB commit and is unregistered only after the final run status/events are persisted, avoiding an intermediate `active=false` plus persisted `running` status window; the registered run is also unregistered if the initial `turn_started` event path fails before orchestration begins, the terminal finalize block unregisters in a `finally` path if assistant-message flush or terminal event persistence fails, and the process-local active registry now reserves event sequence numbers for active runs so a queue/cancel side request cannot reuse a sequence number already emitted but not yet committed by the main run request. This is a small full-stack module for SSE/cancel/replay/error/interruption recovery plus Codex InputQueue-inspired next-input handling; it is not yet multi-worker active-run coordination, hard cancellation of long synchronous model/tool calls, Codex mailbox delivery, true same-turn model-request draining, full SSE Last-Event-ID reconnect, or a Codex-style rollout persistence model.
...
- Added the durable event-storage slice for the same Codex-inspired runtime path: Alembic revision `20260521_0014` creates `agent_run_events`, `DurableAgentRunEventSink` now persists the exact live event stream while optionally forwarding it to SSE, and `GET /api/assistant/runs/{run_id}/events` reads persisted events first before falling back to reconstructed legacy events. This makes post-run replay match the live process stream more closely, while cancellation remains process-local until a future durable active-turn/queue layer exists. Verified with targeted Agent Runtime v2 API tests, migration SQL generation, and `git diff --check`.
- Connected the durable event-storage slice through the frontend replay path: `apps/web/lib/api.ts` now exposes `getAgentRunEvents`, `/agent` fetches the latest run's persisted events and uses them in the process panel after live streaming finishes, and Playwright fixtures/tests assert the process panel renders replayed event titles such as `生成给用户的回答`. Verified with `pnpm lint:web`, `pnpm build:web`, workspace Playwright tests, and `git diff --check`.

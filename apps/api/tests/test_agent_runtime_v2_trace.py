...
from app.runtime.v2.schemas import EvidenceRef, WorkerOutput
...
def test_agent_runtime_v2_persists_trace_and_exposes_owned_trace(
    ...
    plan_step = next(
        step for step in trace_payload["steps"] if step["step_name"] == "build_agent_plan"
    )
    ...
    assert concept_signal["source"] == "scored_search"
    ...
    worker_step = next(
        step for step in trace_payload["steps"] if step["step_name"] == "execute_workers"
    )
    ...
    with session_factory() as session:
        ...
        assert {
            event["name"] for event in run.tool_trace["trace_events"]
        } >= {"agent.plan", "agent.tools", "agent.guardrails"}
